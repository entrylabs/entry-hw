...
Module.prototype.calculateCirclePulse = function(deg, radius, offset) {
	return Math.round(deg * (Brown.DEG_TO_PULSE + offset) * (radius + this.alignment.distance) / 360.0 / this.alignment.distance);
};
				...
				if(unit == 3) { // UNIT_PULSE
					...
					switch(motion.type) {
						...
						case 2: // MOTION_MOVE_BACKWARD
							wheel.pulse = Math.round(value * Brown.CM_TO_PULSE);
							break;
						case 3: // MOTION_TURN_LEFT
							wheel.pulse = Math.round(value * (Brown.DEG_TO_PULSE + alignment.spinLeft) / 360.0);
							break;
						case 4: // MOTION_TURN_RIGHT
							wheel.pulse = Math.round(value * (Brown.DEG_TO_PULSE + alignment.spinRight) / 360.0);
							break;
						case 5: // MOTION_PIVOT_LEFT_FORWARD
						case 6: // MOTION_PIVOT_LEFT_BACKWARD
							wheel.pulse = Math.round(value * (Brown.DEG_TO_PULSE_PIVOT + alignment.pivotLeft) / 360.0);
							break;
						case 7: // MOTION_PIVOT_RIGHT_FORWARD
						case 8: // MOTION_PIVOT_RIGHT_BACKWARD
							wheel.pulse = Math.round(value * (Brown.DEG_TO_PULSE_PIVOT + alignment.pivotRight) / 360.0);
							break;
