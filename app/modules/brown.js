...
function Module() {
	...
	this.alignment = {
		state: 0,
		count: 0,
		distance: Brown.WHEEL_CENTER_DISTANCE,
		spinRight: 0,
		pivotRight: 0,
		spinLeft: 0,
		pivotLeft: 0,
		offset: 0,
		cmToPulse: Brown.CM_TO_PULSE,
		cmOffset: 0
	};
...
Module.prototype.parseAlignment = function(data) {
	...
	var moveForwardCm = Brown.MOVE_FORWARD_CM + alignment.cmOffset / 10000.0;
	if(moveForwardCm <= 0) moveForwardCm = Brown.MOVE_FORWARD_CM;
	alignment.cmToPulse = Brown.CM_TO_PULSE * Brown.MOVE_FORWARD_CM / moveForwardCm;
	str = data.slice(18, 24);
	value = parseInt(str, 16);
	if(value > 0x7fffff) value -= 0x1000000;
	alignment.offset = value + 1;
	alignment.distance = Brown.WHEEL_CENTER_DISTANCE + alignment.offset / 10000.0;
	if(alignment.distance <= 0) alignment.distance = Brown.WHEEL_CENTER_DISTANCE;
};
...
Module.prototype.handleRemoteData = function(handler) {
	...
	var t;
	// left wheel
	if(handler.e(Brown.LEFT_WHEEL)) {
		t = handler.read(Brown.LEFT_WHEEL);
		if(t < -100) t = -100;
		else if(t > 100) t = 100;
		motoring.leftWheel = t;
	}
	// right wheel
	if(handler.e(Brown.RIGHT_WHEEL)) {
		t = handler.read(Brown.RIGHT_WHEEL);
		if(t < -100) t = -100;
		else if(t > 100) t = 100;
		motoring.rightWheel = t;
	}
	// led
	if(handler.e(Brown.LED_RED)) {
		t = handler.read(Brown.LED_RED);
		if(t < 0) t = 0;
		else if(t > 255) t = 255;
		motoring.ledRed = t;
	}
	if(handler.e(Brown.LED_GREEN)) {
		t = handler.read(Brown.LED_GREEN);
		if(t < 0) t = 0;
		else if(t > 255) t = 255;
		motoring.ledGreen = t;
	}
	if(handler.e(Brown.LED_BLUE)) {
		t = handler.read(Brown.LED_BLUE);
		if(t < 0) t = 0;
		else if(t > 255) t = 255;
		motoring.ledBlue = t;
	}
	// buzzer
	if(handler.e(Brown.BUZZER)) {
		t = handler.read(Brown.BUZZER);
		if(t < 0) t = 0;
		else if(t > 167772.15) t = 167772.15;
		motoring.buzzer = t;
	}
	// pulse
	if(handler.e(Brown.PULSE_ID)) {
		t = handler.read(Brown.PULSE_ID);
		if(t != motoring.pulseId) {
			motoring.pulseId = t;
			if(handler.e(Brown.PULSE)) {
				t = handler.read(Brown.PULSE);
				if(t < 0) t = 0;
	...
	// note
	if(handler.e(Brown.NOTE)) {
		t = handler.read(Brown.NOTE);
		if(t < 0) t = 0;
		else if(t > 88) t = 88;
		motoring.note = t;
	}
	// sound
	if(handler.e(Brown.SOUND_ID)) {
		t = handler.read(Brown.SOUND_ID);
		if(t != motoring.soundId) {
			motoring.soundId = t;
			if(handler.e(Brown.SOUND) && handler.e(Brown.SOUND_REPEAT)) {
				t = handler.read(Brown.SOUND);
				if(t < 0) t = 0;
				else if(t > 127) t = 127;
				var t2 = handler.read(Brown.SOUND_REPEAT);
				this.runSound(t, t2);
			}
		}
	}
	// line tracer mode
	if(handler.e(Brown.LINE_TRACER_MODE_ID)) {
		t = handler.read(Brown.LINE_TRACER_MODE_ID);
		if(t != motoring.lineTracerModeId) {
			motoring.lineTracerModeId = t;
			if(handler.e(Brown.LINE_TRACER_MODE)) {
				t = handler.read(Brown.LINE_TRACER_MODE);
				if(t < 0) t = 0;
				else if(t > 15) t = 15;
	...
	// line tracer speed
	if(handler.e(Brown.LINE_TRACER_SPEED)) {
		t = handler.read(Brown.LINE_TRACER_SPEED);
		if(t < 1) t = 1;
		else if(t > 8) t = 8;
		motoring.lineTracerSpeed = t;
	}
	// motion
	if(handler.e(Brown.MOTION_ID)) {
		t = handler.read(Brown.MOTION_ID);
		if(t != motoring.motionId) {
			motoring.motionId = t;
			if(handler.e(Brown.MOTION_UNIT)) {
				t = handler.read(Brown.MOTION_UNIT);
				if(t < 0) t = 0;
				else if(t > 3) t = 3;
				motoring.motionUnit = t;
			}
			if(handler.e(Brown.MOTION_SPEED)) {
				t = handler.read(Brown.MOTION_SPEED);
				if(t < 0) t = 0;
				else if(t > 100) t = 100;
				motoring.motionSpeed = t;
			}
			if(handler.e(Brown.MOTION_VALUE)) {
				t = handler.read(Brown.MOTION_VALUE);
				if(t < 0) t = 0;
				else if(t > 2147483647) t = 2147483647;
				motoring.motionValue = t;
			}
			if(handler.e(Brown.MOTION_RADIUS)) {
				t = handler.read(Brown.MOTION_RADIUS);
				if(t < 0) t = 0;
				else if(t > 2147483647) t = 2147483647;
				motoring.motionRadius = t;
			}
			if(handler.e(Brown.MOTION_TYPE)) {
				t = handler.read(Brown.MOTION_TYPE);
				if(t < 0) t = 0;
				else if(t > 12) t = 12;
...
Module.prototype.requestLocalData = function() {
				...
				if(unit == 3) { // UNIT_PULSE
					...
					switch(motion.type) {
						...
						case 3: // MOTION_TURN_LEFT
							wheel.pulse = Math.round(value * (Brown.DEG_TO_PULSE_SPIN_LEFT + alignment.spinLeft) / 360.0);
							break;
						case 4: // MOTION_TURN_RIGHT
							wheel.pulse = Math.round(value * (Brown.DEG_TO_PULSE_SPIN_RIGHT + alignment.spinRight) / 360.0);
							break;
						case 5: // MOTION_PIVOT_LEFT_FORWARD
						case 6: // MOTION_PIVOT_LEFT_BACKWARD
							wheel.pulse = Math.round(value * (Brown.DEG_TO_PULSE_PIVOT_LEFT + alignment.pivotLeft) / 360.0);
							break;
						case 7: // MOTION_PIVOT_RIGHT_FORWARD
						case 8: // MOTION_PIVOT_RIGHT_BACKWARD
							wheel.pulse = Math.round(value * (Brown.DEG_TO_PULSE_PIVOT_RIGHT + alignment.pivotRight) / 360.0);
							break;
						case 9: // MOTION_CIRCLE_LEFT_FORWARD
						case 10: // MOTION_CIRCLE_LEFT_BACKWARD
							wheel.pulse = self.calculateCirclePulse(Brown.DEG_TO_PULSE_SPIN_LEFT, value, motoring.motionRadius, alignment.spinLeft);
							break;
						case 11: // MOTION_CIRCLE_RIGHT_FORWARD
						case 12: // MOTION_CIRCLE_RIGHT_BACKWARD
							wheel.pulse = self.calculateCirclePulse(Brown.DEG_TO_PULSE_SPIN_RIGHT, value, motoring.motionRadius, alignment.spinRight);
							break;
						default:
							motion.type = 0;
							break;
	...
	if(speed == 0) speed = Brown.DEFAULT_SPEED;
	switch(motion.type) {
		case 1: // MOTION_MOVE_FORWARD
			leftWheel = speed;
			rightWheel = speed;
