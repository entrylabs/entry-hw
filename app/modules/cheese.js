...
Module.prototype.getOrCreateReadHatArray = function() {
	...
	if(sensory.readHat == undefined) sensory.readHat = new Array(20);
	return sensory.readHat;
};

Module.prototype.getOrCreateWriteHatArray = function() {
	var motoring = this.motoring;
	if(motoring.writeHat == undefined) motoring.writeHat = new Array(20);
	return motoring.writeHat;
};

Module.prototype.getOrCreateReadPidArray = function() {
	var sensory = this.sensory;
	if(sensory.readPid == undefined) sensory.readPid = new Array(20);
	return sensory.readPid;
};

Module.prototype.getOrCreateWritePidArray = function() {
	var motoring = this.motoring;
	if(motoring.writePid == undefined) motoring.writePid = new Array(20);
	return motoring.writePid;
};

Module.prototype.getOrCreateWriteNeopixelArray = function() {
	var motoring = this.motoring;
	if(motoring.writeNeopixel == undefined) motoring.writeNeopixel = new Array(20);
	...
	if(value == 1) { // normal
		...
		switch(motoring.modeSa) {
			case Cheese.MODE_S_BUTTON_INPUT:
				if(value == 0) sensory.inputSa = 1;
				else sensory.inputSa = 0;
				break;
		...
		switch(motoring.modeSb) {
			case Cheese.MODE_S_BUTTON_INPUT:
				if(value == 0) sensory.inputSb = 1;
				else sensory.inputSb = 0;
				break;
			case Cheese.MODE_S_VOLTAGE_INPUT:
		...
		switch(motoring.modeSc) {
			case Cheese.MODE_S_BUTTON_INPUT:
				if(value == 0) sensory.inputSc = 1;
				else sensory.inputSc = 0;
				break;
			case Cheese.MODE_S_VOLTAGE_INPUT:
		...
		switch(motoring.modeLa) {
			case Cheese.MODE_L_BUTTON_INPUT:
				if(value == 0) sensory.inputLa = 1;
				else sensory.inputLa = 0;
				break;
		...
		switch(motoring.modeLb) {
			case Cheese.MODE_L_BUTTON_INPUT:
				if(value == 0) sensory.inputLb = 1;
				else sensory.inputLb = 0;
				break;
			case Cheese.MODE_L_VOLTAGE_INPUT:
		...
		switch(motoring.modeLc) {
			case Cheese.MODE_L_BUTTON_INPUT:
				if(value == 0) sensory.inputLc = 1;
				else sensory.inputLc = 0;
				break;
			case Cheese.MODE_L_VOLTAGE_INPUT:
		...
		var readHat = this.getOrCreateReadHatArray();
		...
		if(motoring.modePid > 0) {
			var readPid = this.getOrCreateReadPidArray();
			for(var i = 0, j = 0; i < 20; ++i, j += 2) {
				str = data.slice(j, j + 2);
	...
	if(handler.e(Cheese.SOUND_ID)) {
		...
		if(t != motoring.soundId) {
			...
			if(handler.e(Cheese.SOUND) && handler.e(Cheese.SOUND_REPEAT)) {
				...
				var t2 = handler.read(Cheese.SOUND_REPEAT);
				this.runSound(t, t2);
			}
		}
	}
	...
	if(handler.e(Cheese.WRITE_HAT_ID)) {
		...
		if(t != motoring.writeHatId) {
			...
			if(handler.e(Cheese.WRITE_HAT)) {
				var writeHat = this.getOrCreateWriteHatArray();
	...
	if(handler.e(Cheese.WRITE_PID_ID)) {
		...
		if(t != motoring.writePidId) {
			...
			if(handler.e(Cheese.WRITE_PID)) {
				var writePid = this.getOrCreateWritePidArray();
	...
	if(handler.e(Cheese.WRITE_NEOPIXEL_ID)) {
		...
		if(t != motoring.writeNeopixelId) {
			...
			if(handler.e(Cheese.WRITE_NEOPIXEL)) {
				var writeNeopixel = this.getOrCreateWriteNeopixelArray();
...
Module.prototype.requestLocalData = function() {
	...
	if(self.hat.written && (self.packetSent != Cheese.PACKET_HAT)) {
		...
		var writeHat = motoring.writeHat;
	...
	if(self.pid.written && (port.modePid == motoring.modePid) && (self.packetSent != Cheese.PACKET_PID)) {
		...
		var writePid = motoring.writePid;
		if(writePid) {
			var str = '';
			for(var i = 0; i < 20; ++i) {
	...
	if(self.neopixel.written && (self.packetSent != Cheese.PACKET_NEOPIXEL)) {
		...
		var writeNeopixel = motoring.writeNeopixel;
...
//		case Cheese.MODE_S_DIGITAL_INPUT_PULL_DOWN_2M: == MODE_S_MAKEY_INPUT
		...
		case Cheese.MODE_S_DIGITAL_INPUT_PULL_DOWN_50K: break;
		case Cheese.MODE_S_ANALOG_INPUT_RELATIVE:
		case Cheese.MODE_S_ANALOG_INPUT_ABSOLUTE:
		case Cheese.MODE_S_VOLTAGE_INPUT: tmp |= 0x01; break;
		case Cheese.MODE_SC_PULSE_INPUT_PULL_DOWN_2M:
		case Cheese.MODE_SC_PULSE_INPUT_PULL_UP_50K:
		case Cheese.MODE_SC_PULSE_INPUT_PULL_DOWN_50K: break;
		case Cheese.MODE_S_PWM_OUTPUT:
		case Cheese.MODE_S_DIGITAL_OUTPUT: tmp |= 0x02; break;
		case Cheese.MODE_S_SERVO_OUTPUT: tmp |= 0x03; break;
		case Cheese.MODE_SA_NEOPIXEL_OUTPUT: tmp |= 0x80; break;
	}
	switch(motoring.modeSb) {
//		case Cheese.MODE_S_DIGITAL_INPUT_PULL_DOWN_2M: == MODE_S_MAKEY_INPUT
		case Cheese.MODE_S_MAKEY_INPUT:
		...
		case Cheese.MODE_S_ANALOG_INPUT_RELATIVE:
		case Cheese.MODE_S_ANALOG_INPUT_ABSOLUTE:
		case Cheese.MODE_S_VOLTAGE_INPUT: tmp |= 0x04; break;
		case Cheese.MODE_SC_PULSE_INPUT_PULL_DOWN_2M:
		case Cheese.MODE_SC_PULSE_INPUT_PULL_UP_50K:
		case Cheese.MODE_SC_PULSE_INPUT_PULL_DOWN_50K: break;
		case Cheese.MODE_S_PWM_OUTPUT:
		case Cheese.MODE_S_DIGITAL_OUTPUT: tmp |= 0x08; break;
		case Cheese.MODE_S_SERVO_OUTPUT: tmp |= 0x0c; break;
		case Cheese.MODE_SA_NEOPIXEL_OUTPUT: break;
	}
	switch(motoring.modeSc) {
//		case Cheese.MODE_S_DIGITAL_INPUT_PULL_DOWN_2M: == MODE_S_MAKEY_INPUT
		case Cheese.MODE_S_MAKEY_INPUT:
		...
		case Cheese.MODE_S_ANALOG_INPUT_RELATIVE:
		case Cheese.MODE_S_ANALOG_INPUT_ABSOLUTE:
		case Cheese.MODE_S_VOLTAGE_INPUT: tmp |= 0x10; break;
		case Cheese.MODE_SC_PULSE_INPUT_PULL_DOWN_2M:
		case Cheese.MODE_SC_PULSE_INPUT_PULL_UP_50K:
		case Cheese.MODE_SC_PULSE_INPUT_PULL_DOWN_50K: tmp |= 0x40; break;
		case Cheese.MODE_S_PWM_OUTPUT:
		case Cheese.MODE_S_DIGITAL_OUTPUT: tmp |= 0x20; break;
		case Cheese.MODE_S_SERVO_OUTPUT: tmp |= 0x30; break;
		case Cheese.MODE_SA_NEOPIXEL_OUTPUT: break;
	}
	str += self.toHex(tmp);
...
//		case Cheese.MODE_L_DIGITAL_INPUT_PULL_DOWN_2M: == MODE_L_MAKEY_INPUT
		...
		case Cheese.MODE_L_DIGITAL_INPUT_PULL_DOWN_50K: break;
		case Cheese.MODE_L_ANALOG_INPUT_RELATIVE:
		case Cheese.MODE_L_ANALOG_INPUT_ABSOLUTE:
		case Cheese.MODE_L_VOLTAGE_INPUT: tmp |= 0x01; break;
		case Cheese.MODE_LC_PULSE_INPUT_PULL_DOWN_2M:
		case Cheese.MODE_LC_PULSE_INPUT_PULL_UP_50K:
		case Cheese.MODE_LC_PULSE_INPUT_PULL_DOWN_50K: break;
		case Cheese.MODE_L_PWM_OUTPUT:
		case Cheese.MODE_L_DIGITAL_OUTPUT: tmp |= 0x02; break;
		case Cheese.MODE_L_SERVO_OUTPUT: tmp |= 0x03; break;
	}
	switch(motoring.modeLb) {
//		case Cheese.MODE_L_DIGITAL_INPUT_PULL_DOWN_2M: == MODE_L_MAKEY_INPUT
		case Cheese.MODE_L_MAKEY_INPUT:
		...
		case Cheese.MODE_L_ANALOG_INPUT_RELATIVE:
		case Cheese.MODE_L_ANALOG_INPUT_ABSOLUTE:
		case Cheese.MODE_L_VOLTAGE_INPUT: tmp |= 0x04; break;
		case Cheese.MODE_LC_PULSE_INPUT_PULL_DOWN_2M:
		case Cheese.MODE_LC_PULSE_INPUT_PULL_UP_50K:
		case Cheese.MODE_LC_PULSE_INPUT_PULL_DOWN_50K: break;
		case Cheese.MODE_L_PWM_OUTPUT:
		case Cheese.MODE_L_DIGITAL_OUTPUT: tmp |= 0x08; break;
		case Cheese.MODE_L_SERVO_OUTPUT: tmp |= 0x0c; break;
	}
	switch(motoring.modeLc) {
//		case Cheese.MODE_L_DIGITAL_INPUT_PULL_DOWN_2M: == MODE_L_MAKEY_INPUT
		case Cheese.MODE_L_MAKEY_INPUT:
		...
		case Cheese.MODE_L_ANALOG_INPUT_RELATIVE:
		case Cheese.MODE_L_ANALOG_INPUT_ABSOLUTE:
		case Cheese.MODE_L_VOLTAGE_INPUT: tmp |= 0x10; break;
		case Cheese.MODE_LC_PULSE_INPUT_PULL_DOWN_2M:
		case Cheese.MODE_LC_PULSE_INPUT_PULL_UP_50K:
		case Cheese.MODE_LC_PULSE_INPUT_PULL_DOWN_50K: tmp |= 0x40; break;
		case Cheese.MODE_L_PWM_OUTPUT:
		case Cheese.MODE_L_DIGITAL_OUTPUT: tmp |= 0x20; break;
		case Cheese.MODE_L_SERVO_OUTPUT: tmp |= 0x30; break;
	}
	str += self.toHex(tmp);
	...
	switch(motoring.modeMab) {
		...
		case Cheese.MODE_MAB_SOUND:
			switch(motoring.modeMab) {
				case Cheese.MODE_M_DIGITAL_OUTPUT: break;
				case Cheese.MODE_M_DC: tmp |= 0x01; break;
				case Cheese.MODE_M_SERVO_DUAL: tmp |= 0x02; break;
				case Cheese.MODE_MAB_SOUND: tmp |= 0x03; break;
			}
			switch(motoring.modeMcd) {
				case Cheese.MODE_M_DIGITAL_OUTPUT: break;
				case Cheese.MODE_M_DC: tmp |= 0x04; break;
				case Cheese.MODE_M_SERVO_DUAL: tmp |= 0x08; break;
			}
			break;
		case Cheese.MODE_M_SERVO_MONO: tmp |= 0x40; break;
		case Cheese.MODE_M_STEP_SW_OFF: tmp |= 0x80; break;
		case Cheese.MODE_M_STEP_SW_WAVE: tmp |= 0x90; break;
		case Cheese.MODE_M_STEP_SW_FULL: tmp |= 0xa0; break;
		case Cheese.MODE_M_STEP_HW_OFF: tmp |= 0xc0; break;
		case Cheese.MODE_M_STEP_HW_WAVE: tmp |= 0xd0; break;
		case Cheese.MODE_M_STEP_HW_FULL: tmp |= 0xe0; break;
	}
	str += self.toHex(tmp);
...
//		case Cheese.MODE_L_DIGITAL_INPUT_PULL_DOWN_2M: == MODE_L_MAKEY_INPUT
	...
	switch(motoring.sound) {
		case Cheese.SOUND_OFF: sndid = 0x00; break;
		case Cheese.SOUND_BEEP: sndid = 0x01; break;
		case Cheese.SOUND_RANDOM_BEEP: sndid = 0x05; break;
		case Cheese.SOUND_NOISE: sndid = 0x07; break;
		case Cheese.SOUND_SIREN: sndid = 0x09; break;
		case Cheese.SOUND_ENGINE: sndid = 0x0b; break;
		case Cheese.SOUND_CHOP: sndid = 0x12; break;
		case Cheese.SOUND_ROBOT: sndid = 0x20; break;
		case Cheese.SOUND_DIBIDIBIDIP: sndid = 0x21; break;
		case Cheese.SOUND_GOOD_JOB: sndid = 0x23; break;
		case Cheese.SOUND_HAPPY: sndid = 0x30; break;
		case Cheese.SOUND_ANGRY: sndid = 0x31; break;
		case Cheese.SOUND_SAD: sndid = 0x32; break;
		case Cheese.SOUND_SLEEP: sndid = 0x33; break;
		case Cheese.SOUND_MARCH: sndid = 0x34; break;
		case Cheese.SOUND_BIRTHDAY: sndid = 0x35; break;
	}
	if(sound.written) {
		sound.written = false;
		if(sndid > 0) {
