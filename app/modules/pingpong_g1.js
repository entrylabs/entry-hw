...
class pingpong_g1 extends BaseModule {
	...
	makePackets(method) {
		...
		if (method === 'connect') {
			...
			result = Buffer.from([0xff,0xff,0x00,0x07, 0x00,0x00, 0xce, 0x00,0x0e,
								0x02, 0x00, 0x00, 0x07, 0x50]);
		} else if (method === 'getSensorData') {
			result = Buffer.from([0xff,0xff,0xff,0xff,
								0x00,0xc8, /* continuous sampling*/
								0xb8, 0x00,0x0b,
								10, 0x01]);
		}
		return result;
	...
	*/
    ...
    handleRemoteData(handler) {
		...
		if (this.send_cmd) {
			if (this.send_cmd.id > this.cmd_seq) {
				...
				//console.log('P:handle RD: %s ', this.dbgHexstr(sendBuffer));
    ...
    handleLocalData(data) {
		...
		if (data.length >= 9) {
			...
			if (data.length === packet_size && opcode === 0xB8) {
				...
				var yy = Math.max(Math.min( data.readInt8(16), 90),-90); yy *= -1;
    ...
    connect() {
		...
		setTimeout(()=> {
			this.sp.write(this.makePackets('getSensorData'), (err)=> { console.log('done.........'); });
		}, 2000);
	}
