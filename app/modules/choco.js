...
const { dataType } = require('./byrobot_drone_8');
...
function escape_encode(data) {
    let buffer = Buffer.alloc(data.length*2);
    let idx = 0;    
    for(let d of data) {
        if(d===0x7C) { buffer[idx] = 0x7D; buffer[idx+1] = 0x5C; idx+=2;}
        else if(d===0x7D) { buffer[idx] = 0x7D; buffer[idx+1] = 0x5D; idx+=2;}
        else if(d===0x7E) { buffer[idx] = 0x7D; buffer[idx+1] = 0x5E; idx+=2;}
        else { buffer[idx] = d; idx++;}
    }
    return buffer.slice(0, idx);
}
function escape_decode(data) {
    let buffer = Buffer.alloc(data.length);
    let idx = 0;    
    for(let i = 0 ; i<data.length; ) {
        if(data[i] === 0x7D) { buffer[idx++] = data[i+1] ^ 0x20; i+=2;}        
        else buffer[idx++] = data[i++];
    }
    return buffer.slice(0, idx);
}

const PROTOCOL_LEN_STX = 1;
const PROTOCOL_LEN_CMD = 1;
const PROTOCOL_LEN_SEQ = 1;
const PROTOCOL_LEN_CRC = 2;
const PROTOCOL_LEN_ETX = 1;
const PROTOCOL_LEN_DEFAULT = PROTOCOL_LEN_STX + PROTOCOL_LEN_CMD + 
                PROTOCOL_LEN_SEQ + PROTOCOL_LEN_CRC + PROTOCOL_LEN_ETX;
    ...
    makeData(msg, args={}) {
        ...
        switch(msg) {
            case "ping":
                ...
                crc = cal_crc16(data);
                encodedCmd = escape_encode(Buffer.concat([data, Buffer.from([crc&0xFF, (crc>>8)&0xFF])]));
                return encodedCmd;
            ...
            case COMMAND_TYPE.MOVE_FORWARD:
            case "move_fw":
            ...
            case COMMAND_TYPE.MOVE_BACKWARD:
            case "move_bw":
