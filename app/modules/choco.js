  ...
  */
  ...
  cal_led_col(args) {
    ...
    if(args.param1==='right') {
      ...
      left_led = args.param2;
    } else if(args.param1==='dual') {
      right_led = args.param2;
      left_led = args.param2;
    }

    return {right_led, left_led};
  }
    ...
    switch (type) {
     ...
     case "ping2":
      ...
      case "move_forward":
        ...
        data.writeUInt32LE(this.cal_move_val(args), 2);
        crc = this.cal_crc16(data);
        encodedCmd = this.escape_encode(Buffer.concat([data, Buffer.from([crc & 0xFF, (crc >> 8) & 0xFF])]));
        break;
      ...
      case "turn_left":
        ...
        data.writeUInt32LE(this.cal_turn_val(args), 2);
        crc = this.cal_crc16(data);
        encodedCmd = this.escape_encode(Buffer.concat([data, Buffer.from([crc & 0xFF, (crc >> 8) & 0xFF])]));
        break;
      ...
      case "move_right_left":  {
          ...
          let args1 = {param1:args.param1, param2:args.param2};
          let args2 = {param1:args.param3, param2:args.param4};
          data.writeUInt32LE(this.cal_move_val(args1), 2);
          data.writeUInt32LE(this.cal_move_val(args2), 6);        
          crc = this.cal_crc16(data);
          encodedCmd = this.escape_encode(Buffer.concat([data, Buffer.from([crc & 0xFF, (crc >> 8) & 0xFF])]));
        }
        break;
      ...
      case "onoff_led_rear":  {
          let rear_led = (args.param1==='On')?1:0;
          data = Buffer.from([0x0B, seqNo, 0, 0, rear_led]);          
          crc = this.cal_crc16(data);
          encodedCmd = this.escape_encode(Buffer.concat([data, Buffer.from([crc & 0xFF, (crc >> 8) & 0xFF])]));
        }
        break;
