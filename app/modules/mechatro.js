...
Module.prototype.dataFromEntry = function(handler){
    ...
    for (let i = 0; i < this.portList.length ; i++) {  // 사용하는 포트 리스트에 대하여 없데이트를 한다.
        //var str="";
        portNo = this.portList[i];
        mPortNo = "m" + portNo;

        // 모드 저장
        if ( handler.e(mPortNo) == true ){            // 포트에 대한 모드 변수가 있는지 확인한다.
            mode = handler.read(mPortNo);
            if( this.contMode[portNo] != mode ){      // 현재값과 다른 경우에 업데이트 한다.
        ...
        // 값 저장
        if ( handler.e(portNo) == true ){             // 포트 번호가 있는지 확인한다.
            value = handler.read(portNo);
            if( this.entryValue[portNo] != value ){   // 현재값과 다른 경우에 업데이트 한다.
