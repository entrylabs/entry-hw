...
class PingpongBase extends BaseModule {
    constructor(cube_cnt) {
        ...
        this.cubeCount = cube_cnt || 2;

        console.log('PINGPONG construct : G' + this.cubeCount);
    }
        ...
        if (data.length >= 9) {
            ...
            if (data.length >= packet_size && packet_size >= 19) {
                if (opcode == 0xb8 && data[5] == 0xc8) {
                    ...
                    if (cubeid >= this.cubeCount) return;
                    sensor = this._sensorData[cubeid];
