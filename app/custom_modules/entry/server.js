...
Server.prototype.open = function(logger) {
    ...
    if (fs.existsSync(path.resolve(global.__dirname, 'ssl', 'cert.pem'))) {
        ...
        httpServer = http.createServer(
            {
                key: fs.readFileSync(
                    path.resolve(global.__dirname, 'ssl', 'hardware.key')
                ),
                cert: fs.readFileSync(
                    path.resolve(global.__dirname, 'ssl', 'cert.pem')
                ),
                ca: [
                    fs.readFileSync(
                        path.resolve(global.__dirname, 'ssl', 'ChainCA1.crt')
                    ),
                    fs.readFileSync(
                        path.resolve(global.__dirname, 'ssl', 'ChainCA2.crt')
                    ),
                    fs.readFileSync(
                        path.resolve(global.__dirname, 'ssl', 'RootCA.crt')
                    ),
                ],
            },
            function(req, res) {
        ...
        server.on('connection', function(socket) {
            ...
            connection.on('disconnect', function(socket) {
                if (connection.handshake.query.childServer === 'true') {
                    ...
                    var childServerListCnt = Object.keys(self.childServerList)
                        .length;
                    if (childServerListCnt <= 0) {
                        server.emit('mode', serverModeTypes.single);
                        ipcRenderer.send('serverMode', serverModeTypes.single);
...
Server.prototype.send = function(data) {
    ...
    if (
        (runningMode === serverModeTypes.parent && childServerListCnt === 0) ||
        runningMode === serverModeTypes.child
    ) {
        ...
        masterRoomIds.forEach(function(masterRoomId) {
            self.server
                .to(masterRoomId)
                .emit('message', { data: data, version: version });
        });
    }
};
