...
import { SERVER_MODE_TYPES } from '../common/constants';
...
interface SocketIoClientSocket extends SocketIOClient.Socket {
    mode?: number;
}

interface ConnectionSocket extends Socket {
    roomId?: string;
    roomIds?: string[];
}
...
class EntryServer extends EventEmitter {
    ...
    connectionSet: { [key: string]: ConnectionSocket };
    roomCnt: number;
    childServerList: { [key: string]: boolean };
    clientTargetList: { [key: string]: string };
    runningMode: number;
    currentServerMode: number;
    masterRoomIds: string[];
    clientRoomId: string;
    httpServer?: Http.Server | Https.Server;
    socketClient?: SocketIoClientSocket; // 클라이언트인 경우 세팅됨
    socketServer?: SocketIoServer.Server; // 호스트인 경우 세팅됨
    state: string;
    ...
    open() {
        ...
        httpServer.on('listening', () => {
            ...
            if (mRoomIds.length > 0) {
                mRoomIds.forEach((mRoomId: string) => {
     ...
     * @private
     */
    _createSocketClient(address: string) {
        const socket: SocketIoClientSocket = SocketIoClient(address, {
        ...
        server.on('connection', (socket: ConnectionSocket) => {
            ...
            connection.on('message', (message, ack) => {
                ...
                if (
                    ...
                    (connection.roomId && this.masterRoomIds.indexOf(connection.roomId) >= 0)
                ) {
                    ...
                    } else if (connection.roomId && this.clientTargetList[connection.roomId]) {
            ...
            connection.on('close', (reasonCode, description) => {
                ...
                this.emit('close');
                this.closeSingleConnection(connection);
            });
    ...
    closeSingleConnection(connection: ConnectionSocket | SocketIoClientSocket) {
        const connections = this.connections;
        const index = connections.indexOf(connection);
        if (index > -1) {
            this.connections.slice(index, 1);
            if ((connection as ConnectionSocket).disconnect) {
                (connection as ConnectionSocket).disconnect();
            } else if ((connection as SocketIoClientSocket).close) {
                (connection as SocketIoClientSocket).close();
            }
        }
    };
    ...
    send(data: any) {
        ...
        if (
            ...
            this.masterRoomIds.forEach((masterRoomId) => {
                this.socketServer && this.socketServer.to(masterRoomId).emit('message', payload);
            });
        }
    };
