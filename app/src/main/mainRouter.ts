...
import HardwareListManager from './hardwareListManager';
import * as HandlerCreator from './datahandler/handler';
...
class MainRouter {
    ...
    constructor(mainWindow: BrowserWindow) {
        ...
        ipcMain.on('requestFlash', (e: Electron.Event, firmwareName: string) => {
        ...
        ipcMain.on('executeDriver', (e: Electron.Event, driverPath: string) => {
            this.executeDriver(driverPath);
        });
        ipcMain.on('getCurrentServerModeSync', (e: Electron.Event) => {
            e.returnValue = this.server.currentServerMode;
        });
        ipcMain.on('requestHardwareListSync', (e: Electron.Event) => {
    ...
    handleServerData({ data, type }: { data: any, type: any }) {
        const hwModule = this.hwModule;
