...
class Scanner {
     ...
     * @returns {boolean}
     */
    _indexOfStringOrArray(arrayOrString?: string | string[], target?: string) {
    ...
    getComPortList(): Promise<SerialPortType.PortInfo[]> {
        return new Promise((resolve, reject) => {
            // noinspection JSIgnoredPromiseFromCall
            SerialPort.list((error: Error, list: SerialPortType.PortInfo[]) => {
                if (error) {
                    reject(error);
                } else {
