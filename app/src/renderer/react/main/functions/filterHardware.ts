...
const filterHardwareList = (keyword: string, category: string, hardwareList: IHardwareConfig[]) => hardwareList.filter((hardware) => (
            categoryFilterHardware(hardware, category) &&
            nameFilterHardware(hardware, keyword)
        ));

const categoryFilterHardware = (hardwareItem: IHardwareConfig, category: string) => category === CategoryTypeEnum.all || hardwareItem.category === category;
