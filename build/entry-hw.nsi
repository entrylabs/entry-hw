...
Section $(TEXT_ENTRY_TITLE) SectionEntry
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Entry_HW" "UninstallString" '"$INSTDIR\uninstall.exe"'
...
Section $(TEXT_DESKTOP_TITLE) SectionDesktop
  ...
  CreateShortCut "$DESKTOP\엔트리_하드웨어.lnk" "$INSTDIR\nw.exe" "" "$INSTDIR\icon.ico" 0
...
Section "Uninstall"
  ...
  Delete "$SMPROGRAMS\엔트리_하드웨어\*.*"
  
  Delete "$DESKTOP\엔트리_하드웨어.lnk"

  ; Remove directories used
  RMDir "$SMPROGRAMS\엔트리_하드웨어"
  RMDir /r "$INSTDIR"
