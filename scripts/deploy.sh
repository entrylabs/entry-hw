...
git push --delete "https://${ACCESS_TOKEN}@github.com/entrylabs/entry-hw" "$targetBranchName"

echo "forceful add files to git"
git add -f .

git commit -m "Entry-HW for electron embed into $targetBranchName"
git push --force --quiet "https://${ACCESS_TOKEN}@github.com/entrylabs/entry-hw" "$targetBranchName"
